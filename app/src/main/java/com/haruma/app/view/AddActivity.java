...
public class AddActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //đồng hồ
        EditText edtStartTime = findViewById(R.id.edtStartTime);
        EditText edtEndTime = findViewById(R.id.edtEndTime);

        edtStartTime.setOnClickListener(v -> showTimePickerDialog(edtStartTime , true));
        edtEndTime.setOnClickListener(v -> showTimePickerDialog(edtEndTime , false));
    }
    //Đồng hồ
    private void showTimePickerDialog(EditText targetEditText, boolean isStartTime) {
        int hour = 0;
        int minute = 0;

        // Nếu EditText đã có giờ -> dùng giờ đó
        String timeStr = targetEditText.getText().toString();
        if (timeStr != null && timeStr.matches("\\d{2}:\\d{2}")) {
            String[] parts = timeStr.split(":");
            hour = Integer.parseInt(parts[0]);
            minute = Integer.parseInt(parts[1]);
        } else {
            // Nếu chưa có thì dùng giờ hiện tại
            final Calendar calendar = Calendar.getInstance();
        ...
        TimePickerDialog timePickerDialog = new TimePickerDialog(
                ...
                (view, hourOfDay, minuteOfHour) -> {
                    ...
                    // Nếu bạn có ViewModel, bạn có thể set lại tại đây
                    // addViewModel.setStartTime(selectedTime); hoặc setEndTime
                },
                hour,
                minute,
                true
